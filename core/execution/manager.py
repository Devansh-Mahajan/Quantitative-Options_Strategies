...
from core.execution.delay_aware_options import (
    build_delay_adjusted_contracts,
    effective_ask_price,
    effective_bid_price,
)
from models.contract import Contract
from core.utils import get_option_days_to_expiry, parse_option_symbol, try_parse_option_symbol
from config.params import PROFIT_TARGET, STOP_LOSS
from core.telemetry.state_manager import get_equity_overlay_metadata, get_straddle_metadata, remove_straddle_metadata 
...
def manage_open_spreads(client, profit_target=PROFIT_TARGET, stop_loss=STOP_LOSS):
    ...
    for pos in positions:
        ...
        # NOTE: keyed by underlying+type only, so a second same-type spread on the same
        # underlying would silently overwrite the first entry's short/long leg here.
        # Not currently reachable: scripts/run_strategy.py's concentration gate
        # (active_counts < MAX_SPREADS_PER_SYMBOL * 2) caps each underlying to one
        # open spread (2 legs) at a time. If that gate is ever loosened, this grouping
        # needs to key on a spread identifier (e.g. matched short+long strike pair)
        # rather than underlying+type alone.
